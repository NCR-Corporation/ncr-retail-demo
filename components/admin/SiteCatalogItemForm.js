...
import { Row, Col, ModalHeader, ModalBody, ModalFooter, Button, Alert } from 'reactstrap';
...
const SiteCatalogItemForm = ({ toggle, itemObject, siteId, setShowAlert }) => {
  ...
  const initialValues = {
    ...
    longDescription: itemAttributes.longDescription && itemAttributes.longDescription.values.length > 0 ? itemAttributes.longDescription.values[0].value : '',
    ...
    version: itemAttributes.version > itemPrices[0].version ? itemAttributes.version + 1 : itemPrices[0].version + 1,
    priceId: itemPrices[0].priceId.priceCode
  }

  const handleSubmit = async values => {
    fetch(`/api/sites/catalog/${siteId}/${item.itemId.itemCode}`, { method: 'POST', body: JSON.stringify(values) }).then(response => response.json()).then(data => {
        ...
        return (
          <Form>
            ...
            <ModalFooter className="justify-content-between">
              ...
              <button type="submit" className={`${!(dirty && isValid) ? "disabled" : ""} btn btn-primary`} disabled={`${!(dirty && isValid) ? "disabled" : ""}`}>Update Item</button>
            </ModalFooter>
          </Form>
        )
      }}
