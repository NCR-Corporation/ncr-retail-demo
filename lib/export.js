...
export async function exportAllData() {
  ...
  const promise = sites.map(async (element) => {
    ...
    let itemPrices = await getSiteCatalogItemPricesByItemCodes(
      element.id,
      itemCodesObj
    );
    let itemAttributes = await getSiteCatalogItemAttributesByItemCodes(
      element.id,
      itemCodesObj
    );
    itemPricesArray = itemPricesArray.concat(itemPrices.data.itemPrices);
    itemAttributesArray = itemAttributesArray.concat(
    ...
    sitesFormatted.push({
      ...
      contactPhoneNumberCountryCode: element.contact
        ? element.contact.phoneNumberContryCode
        : '',
