...
import { getCatalogItems, getCatalogItemPricesByItemCodes, getCatalogItemAttributesByItemCodes } from './catalog';
...
export async function exportAllData() {
  ...
  const promise = sites.map(async element => {
    let itemPrices = await getCatalogItemPricesByItemCodes(element.id, itemCodes);
    let itemAttributes = await getCatalogItemAttributesByItemCodes(element.id, itemCodes);
  ...
  // return { csv }
  return { categories, "sites": csv, "items": items, "itemAttributes": itemAttributesResult, "itemPrices": itemPricesResult }
}