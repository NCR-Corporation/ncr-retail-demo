...
export async function getCatalogItemDetails(id, query) {
  if (query) {
    let data = await sendRequest(`${baseUrl}/item-details/search?shortDescriptionPattern=\*${query}\*&itemStatus=ACTIVE&sortField=ITEM_CODE&sortDirection=ASC`, 'GET', null, id);
    return data;
  } else {
    let data = await sendRequest(`${baseUrl}/item-details/search?itemStatus=ACTIVE&sortField=ITEM_CODE&sortDirection=ASC`, 'GET', null, id);
    return data;
  }
...
export async function getCatelogItemsByMerchandiseCategoryId(name, id) {
  let response = await sendRequest(`${baseUrl}/items/?merchandiseCategoryId=${name}&itemStatus=ACTIVE&sortField=ITEM_CODE&sortDirection=ASC`, 'GET');
...
export async function updateCatalogItemPricesByItemCode(siteId, itemCode, priceCode, values) {
  let response = await sendRequest(`${baseUrl}/item-prices/${itemCode}/${priceCode}`, 'PUT', JSON.stringify(values), siteId)
  return response;
}

export async function updateCatalogItemAttributesByItemCode(siteId, itemCode, values) {
  let response = await sendRequest(`${baseUrl}/item-attributes/${itemCode}`, 'PUT', JSON.stringify(values), siteId);
  return response;
}
...
export async function getHomepageCatalogItems() {
  let response = await sendRequest(`${baseUrl}/items?pageSize=10&itemStatus=ACTIVE&sortField=ITEM_CODE&sortDirection=ASC`, 'GET');
  return response;
}