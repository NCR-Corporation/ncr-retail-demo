...
export async function getAllCategoryNodes(flatten = false) {
  ...
  let result = [];
  parentCategories.forEach(category => {
    let notSorted = parentCategoriesMap[category.nodeCode];
...
export async function getCatelogItemsByMerchandiseCategoryId(name, siteId) {
  let response = await sendRequest(`${baseUrl}/item-details/search?merchandiseCategoryId=${name}&itemStatus=ACTIVE&sortField=ITEM_CODE&sortDirection=ASC`, 'GET', null, siteId);
  return response;
}
